...
class SwissMilitaryWatchfaceView extends WatchUi.WatchFace
{
    var font;
    var isAwake;
    var screenShape;
    ...
    function generateHandCoordinates(centerPoint, angle, handLength, tailLength, arrowLength, width) {
        
        // Map out the coordinates of the watch hand
        var handPadding = 2;
        var handCoords = [[-(width / 2), tailLength], [-(width / 2), -handLength], [0, (-handLength - arrowLength)], [width / 2, -handLength], [width / 2, tailLength]];
        var rectCoords = [[-(width / 2)+handPadding, -tailLength], [-(width / 2)+handPadding, -handLength], [(width / 2)-handPadding, -handLength], [(width / 2)-handPadding, -tailLength]];
        ...
        for (var i = 0; i < 5; i += 1) {
            ...
            var xHand = (handCoords[i][0] * cos) - (handCoords[i][1] * sin) + 0.5;
            var yHand = (handCoords[i][0] * sin) + (handCoords[i][1] * cos) + 0.5;
            if(i < 4){
                var xRect = (rectCoords[i][0] * cos) - (rectCoords[i][1] * sin) + 0.5;
                var yRect = (rectCoords[i][0] * sin) + (rectCoords[i][1] * cos) + 0.5;
            }

            result[0][i] = [centerPoint[0] + xHand, centerPoint[1] + yHand];
            result[1][i] = [centerPoint[0] + xRect, centerPoint[1] + yRect];
        }
    ...
    function onUpdate(dc) {
        ...
        var watchHand = generateHandCoordinates(screenCenterPoint, hourHandAngle, 40, 10, 5, 5)
        targetDc.setColor(Graphics.COLOR_BLACK, Graphics.COLOR_TRANSPARENT);
        targetDc.fillPolygon(watchHand[0]);
        targetDc.setColor(Graphics.COLOR_YELLOW, Graphics.COLOR_TRANSPARENT);
        targetDc.fillPolygon(watchHand[1]);
        
        // Draw the minute hand.
        minuteHandAngle = (clockTime.min / 60.0) * Math.PI * 2;
        watchHand = generateHandCoordinates(screenCenterPoint, minuteHandAngle, 70, 10, 5, 5)
        targetDc.setColor(Graphics.COLOR_BLACK, Graphics.COLOR_TRANSPARENT);
        targetDc.fillPolygon(watchHand[0]);
        targetDc.setColor(Graphics.COLOR_YELLOW, Graphics.COLOR_TRANSPARENT);
        targetDc.fillPolygon(watchHand[1]);
